# Placeholder font data. Replace with actual 8x8 font data.
...
# def text(self, s, x0, y0, col=1):
#     for char in s:
#         chr = ord(char)
#         if chr < 32 or chr > 127:
#             chr = 127
#         chr_data = font_petme128_8x8[(chr - 32) * 8: (chr - 32 + 1) * 8]
#         for j in range(8):
#             if x0 + j >= self.width:
#                 break
...
def get_char_data(char):
    ...
    char_data = font_petme128_8x8[start_index:start_index + 8]

    # Convert the character data to a list of lists of bits
    bit_data = []
    for byte in char_data:
        # bits = [1 if byte & (1 << bit) else 0 for bit in range(7, -1, -1)]
...
# Convert each bit string back to a byte
byte_data = [f"0x{bits_to_byte(bits):02x}" for bits in bit_data]
